...
def get_next_batch_from_path(image_path, image_labels, pointer, IMAGE_HEIGHT=299, IMAGE_WIDTH=299, batch_size=64, is_train=True):
    batch_x = np.zeros([batch_size, IMAGE_HEIGHT,IMAGE_WIDTH,3])
    # print('image_labels[0]',image_labels.shape)
    num_classes = int(image_labels.shape[1])
    batch_y = np.zeros([batch_size, num_classes]) 
    for i in range(batch_size):  
        image = cv2.imread(image_path[i+pointer*batch_size])
...
def test():
    ...
    craterDir = "D:\\study\\code_practice\\vscode\\gabage\\train_arch\\images01\\training"
    train_rate=0.8
    all_image = load_image(craterDir, train_rate)
    train_data, train_label, valid_data, valid_label = all_image.gen_train_valid_image()
    # dataset=load_image(craterDir,train_rate)
    # train_data, train_label = dataset.gen_train_valid_image()

    print ('train_data.shape:',train_data.shape)
    print ('len(train_data)',len(train_data))
    print ('train_data[0].shape',train_data[0].shape)
    print ('train_label[0]',train_label[0])
    # batch_x, batch_y = get_next_batch_from_path(train_data, train_label, 0, IMAGE_HEIGHT=299, IMAGE_WIDTH=299, batch_size=64, is_train=True)
